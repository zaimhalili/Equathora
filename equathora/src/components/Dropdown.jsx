...
const Dropdown = ({ label, items, alignRight = false }) => {
    return (
        <div className='relative flex h-[84px] ml-10 lg:ml-5 z-[1001] group'>
            ...
            <div className={`opacity-0 invisible absolute ${alignRight ? 'right-[-60px] lg:right-[-10px]' : 'left-[-60px]'} top-[65px] bg-[var(--main-color)] min-w-[360px] max-w-[360px] lg:min-w-[320px] lg:max-w-[320px] shadow-[0_10px_12px_rgba(0,0,0,0.2)] font-[Public_Sans,sans-serif] rounded-[5px_5px_10px_10px] z-[1002] transition-all duration-200 group-hover:translate-y-2 group-hover:opacity-100 group-hover:visible`}>
                {items.map((item, i) =>
                    item.isButton ? (
                        <button
                            key={i}
