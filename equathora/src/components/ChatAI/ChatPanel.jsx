...
const ChatPanel = forwardRef(
    ({
        ...
        const sendMessage = async (text) => {
            ...
            const updatedHistory = [...chatMessages, userMsg].slice(-MAX_HISTORY_MESSAGES);
            setChatMessages(updatedHistory);
            setIsAiThinking(true);

            try {
                const aiResponseText = await askThoraChat({
                    problemDescription,
                    acceptedAnswer: acceptedSolution,
                    chatHistory: updatedHistory.slice(-20),
                    userNewMessage: text,
                });
                const safeText = sanitize(String(aiResponseText ?? '')).slice(0, 2000) || 'I had trouble generating a response.';
                setChatMessages(prev => [...prev, { id: Date.now(), sender: 'ai', text: safeText }].slice(-MAX_HISTORY_MESSAGES));
            } catch {
                setChatMessages(prev => [...prev, { id: Date.now(), sender: 'ai', text: 'Sorry, something went wrong.' }].slice(-MAX_HISTORY_MESSAGES));
            } finally {
