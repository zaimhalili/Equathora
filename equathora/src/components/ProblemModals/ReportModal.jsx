...
const ReportModal = ({ isOpen, onClose, reportReason, setReportReason, reportDetails, setReportDetails, onSubmit }) => {
    ...
    return (
        <div className='fixed inset-0 flex items-center justify-center z-50 bg-[var(--raisin-black)]/30 backdrop-blur-[2px]' onClick={onClose}>
            <div className='bg-[var(--white)] w-11/12 max-w-md rounded-xl px-6 py-7 flex flex-col shadow-2xl max-h-[85vh] overflow-y-auto' onClick={(e) => e.stopPropagation()}>
                ...
                <div className='flex flex-col gap-4 pt-4'>
                    <div>
                        ...
                        <select
                            ...
                            className='w-full p-3 border-2 text-[var(--secondary-color)] bg-[var(--white)] border-[var(--mid-main-secondary)] rounded-xl  text-sm focus:border-[var(--accent-color)] focus:outline-none transition-colors'
                        >
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows={4}
                            className='w-full p-3 border-2 text-[var(--secondary-color)] bg-[var(--white)] border-[var(--mid-main-secondary)] rounded-xl  text-sm resize-none focus:border-[var(--accent-color)] focus:outline-none transition-colors'
                        />
                    </div>
                </div>
                ...
                <div className='flex w-full justify-between gap-3 pt-6'>
                    ...
                    <button type="button" onClick={onSubmit} className='px-4 cursor-pointer py-2.5 font-bold text-center border-2 border-[var(--accent-color)] rounded-xl bg-[var(--accent-color)] text-[var(--white)] hover:bg-[var(--dark-accent-color)] hover:border-[var(--dark-accent-color)] shadow-md hover:shadow-lg transition-all duration-300 flex-1 text-sm md:text-base'>Submit Report</button>
                </div>
            </div>
        </div>
    );
